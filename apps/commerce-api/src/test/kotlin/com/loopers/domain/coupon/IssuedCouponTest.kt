...
class IssuedCouponTest {
    ...
    inner class UseTest {
        ...
        @DisplayName("사용 가능한 쿠폰을 사용하면 USED 상태로 변경되고 사용 시각이 기록된다")
        @Test
        fun `use available coupon successfully`() {
            ...
            assertAll(
                ...
                { assertThat(issuedCoupon.usedAt).isEqualTo(FIXED_TIME) },
            )
        }
        ...
        fun `throw exception when using different coupon`() {
            ...
            // when & then
            val exception = assertThrows<CoreException> {
                issuedCoupon.use(userId, differentCoupon, FIXED_TIME)
            }
            assertThat(exception.errorType).isEqualTo(ErrorType.BAD_REQUEST)
        ...
        @Test
        fun `throw exception when using coupon owned by another user`() {
            // given
            val ownerId = 1L
            ...
            // when & then
            val exception = assertThrows<CoreException> {
                issuedCoupon.use(anotherUserId, coupon, FIXED_TIME)
            }
            assertThat(exception.errorType).isEqualTo(ErrorType.BAD_REQUEST)
