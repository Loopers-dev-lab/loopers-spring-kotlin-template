...
class CouponService(
    ...
    @Transactional
    fun applyCoupon(userId: Long, couponId: Long?) {
        if (couponId == null) {
            return
        }
    ...
    fun calculateCouponDiscount(userId: Long, couponId: Long?, totalAmount: Long): Long {
        if (couponId == null) {
            return 0
        }
        ...
        val coupon = couponRepository.findById(couponId)
            ?: throw CoreException(ErrorType.NOT_FOUND, "쿠폰을 찾을 수 없습니다: $couponId")
        couponIssueRepository.findBy(userId, couponId)
            ?: throw CoreException(ErrorType.NOT_FOUND, "사용자가 발급 받은 적 없는 쿠폰입니다. userId: $userId, couponId: $couponId")
