...
class LikeController(private val likeFacade: LikeFacade) : LikeApiSpec {
    ...
    @PutMapping("/like")
    override fun like(@RequestBody request: LikeDto.Request): ApiResponse<LikeDto.Response> {
        likeFacade.like(request.userId, request.productId)
        return ApiResponse.success(LikeDto.Response.success())
    }

    @PutMapping("/unlike")
    override fun unlike(@RequestBody request: LikeDto.Request): ApiResponse<LikeDto.Response> {
