...
class ProductMetricModel(
        @Column(name = "ref_product_id", nullable = false) var refProductId: Long,
) : BaseEntity() {
  ...
  /**
   * viewCount를 delta만큼 업데이트
   * @param delta 변경량 (양수: 증가)
   */
  fun updateViewCount(delta: Long) {
    this.viewCount = (this.viewCount + delta).coerceAtLeast(0)
  }
}
