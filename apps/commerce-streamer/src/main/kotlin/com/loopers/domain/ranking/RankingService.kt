 ...
 * 랭킹 도메인 서비스
 *
 * Redis ZSET에 이벤트별 랭킹 점수를 반영하고, 멱등성을 보장합니다.
 */
@Service
@Transactional
class RankingService(
     ...
     * 좋아요 점수 배치 증가
     */
    fun incrementLikeScoreBatch(events: List<LikeScoreEvent>) {
        processLikeScoreBatch(events, isIncrement = true)
    }

    /**
     * 좋아요 점수 배치 감소
     */
    fun decrementLikeScoreBatch(events: List<LikeScoreEvent>) {
        processLikeScoreBatch(events, isIncrement = false)
    }
     ...
     * 주문 완료 점수 배치 증가
     */
    fun incrementOrderScoreBatch(events: List<OrderScoreEvent>) {
        processOrderScoreBatch(events, isIncrement = true)
    }

    /**
     * 주문 취소 점수 배치 감소
     */
    fun decrementOrderScoreBatch(events: List<OrderScoreEvent>) {
        processOrderScoreBatch(events, isIncrement = false)
    }
